...
$Order_date = $Quantity = $Cost = $Product_name = $Vendor_id = $Staff_id = "";
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty(trim($_POST["Order_date"]))) {
        $OrderD_err = "Please enter a valid date.";
    } else {
    ...
    if(empty(trim($_POST["Staff_id"]))) {
        $Staff_id_err = "Please enter a staff id.";
    } else {
    ...
    if(empty(trim($_POST["Vendor_id"]))) {
        $Vendor_id_err = "Please enter a vendor id.";
    } else {
    ...
    if(empty($OrderD_err) && empty($quantity_err) && empty($prodname_err) && empty($Staff_id_err) && empty($Vendor_id_err) && empty($cost_err)){
        ...
        $sql = "INSERT INTO SUPPLIES_ORDER (Order_date , Quantity , Cost, Product_name , Staff_id , Vendor_id ) VALUES (?, ?, ?, ?, ?, ?)";

             
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "ssssdd", $param_Order_date, $param_quantity, $param_cost, $param_prod_name, $param_staff_id, $param_vendor_id);
        ...
        <div class="form-group">
            ...
            <?php
                ...
                <select name="Staff_id" id="Staff_id" class="form-control">';
                  ...
                  if ($result->num_rows >0){
                    echo '<label for="cars">Vendor</label></br>
